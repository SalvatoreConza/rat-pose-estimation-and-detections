...
def add_annotation_CVAT(index, filename, keypoints, bbox, output_folder, data_folder):
    ...
    labels = {'back_croup': 0, 'back_left_knee': 1, 'back_withers': 2, 'back_left_paw': 3, 'back_left_wrist': 4, 'back_midpoint': 5, 'back_right_hip': 6, 'back_right_knee': 7, 'back_right_paw': 8, 'back_right_wrist': 9, 'back_withers': 10, 'chest': 11, 'front_left_elbow': 12, 'front_left_paw': 13, 'front_left_shoulder': 14, 'front_left_wrist': 15, 'front_right_elbow': 16, 'front_right_paw': 17, 'front_right_shoulder': 18, 'front_right_wrist': 19, 'head_midpoint': 20, 'left_ear_base': 21, 'left_ear_tip': 22, 'left_eye': 23, 'lower_jaw': 24, 'nose': 25, 'right_ear_base': 26, 'right_ear_tip': 27, 'right_eye': 28, 'tail_base': 29, 'tail_end': 30, 'tail_lower_midpoint': 31, 'tail_midpoint': 32, 'tail_upper_midpoint': 33, 'throat': 34}
    ...
    with open(os.path.join(data_folder, 'manifest.jsonl'), "a") as f:
        f.write(f"{{\"name\":\"{basename}\",\"extension\":\".jpg\",\"width\":1280,\"height\":720,\"meta\":{{\"related_images\":[]}}}}\n")
...
if __name__ == "__main__":
    ...
    model_detection = YOLO(r"C:\Users\Lund University\Documents\GitHub\bnl-ai\runs\detect\train17\weights\best.pt")  # pretrained YOLO11n model

    with open(r'C:\Users\Lund University\Documents\GitHub\bnl-ai\config\hrnet_w48_384_288.yaml', 'r') as f:
        cfg_w32_384_288 = yaml.load(f, Loader=yaml.SafeLoader)
        cfg_w32_384_288['MODEL']['NUM_JOINTS'] = 35
        ...
        model_pose.load_state_dict(torch.load(r'C:\Users\Lund University\Documents\GitHub\bnl-ai\out\train_pose-250626_230523\snapshot_PoseHRNet-W48_288x384.pth', weights_only=True, map_location=device))
        ...
        for idx, filename in enumerate(filenames):
            ...
            detection_result = model_detection(filename)[0]
            boxes = detection_result.boxes
            if boxes.xyxy.shape[0] == 0:
                print(f"Nessun mouse rilevato in {filename}, salto l'immagine.")
                continue  # Nessun detection, passa oltre

            # Prendi il box con la confidenza più alta
            confidences = boxes.conf
            best_idx = confidences.argmax()
            bbox = boxes.xyxy[best_idx].to('cpu')
            ...
            rel_path = os.path.relpath(filename, args.input)
            safe_name = rel_path.replace(os.sep, "__")
            cv2.imwrite(os.path.join(prediction_folder, safe_name), image)
            # cv2.waitKey(0)
    ...
    with open(r"C:\Users\Lund University\Documents\GitHub\bnl-ai\config\task_base.json", "r") as f:
